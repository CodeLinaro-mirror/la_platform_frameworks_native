...
bool QtiSurfaceExtensionGPP::DynamicEnableInternal(sp<IGraphicBufferProducer>* gbp, bool needReconnect) {
    if (mIsSupported && mConnectedToGpu) {
        ...
        //default value should not be 0x0(FRC OFF) or 0x1(FRC ON),need to other value,choose 0x11.
        property_get("vendor.gpp.frc.enable", valueStr, "0x11");
        int enable = -1;
        size_t pos = 0;
            ...
            if (enable) {
                ...
                if (err == OK) {
                    ...
                    if (err == NAME_NOT_FOUND || err == INVALID_OPERATION) {
                       mIsSupported = false;
                       ALOGE("Failed to init GPP: Surface or App is not supported by GPP");
                    } else {
                        mIsSupported = false;
    ...
    if (mIsEnable == true) {
        switch (what) {
            ...
            case NATIVE_WINDOW_CONSUMER_IS_PROTECTED:
                /* Call the mOriginalGbp directly to avoid unnecessary binder call
                due to those values of mGbp are the same as mOriginalGbp. */
